...
class ScoringRules:
    ...
    ヨットの得点ルールに関する設定. 任天堂の「世界の遊び大全51」で収録されているルールをベースにする.
      ...
      - フルハウス : 同じ出目のグループが2つあり, グループ内の個数が2個と3個であるときに選択可. 点数はその時点の出目の総和.
        - ヨットが成立する出方でも成立する.
...
class YachtEnv(gym.Env):
    ...
    - 実装上は Box(19, ) として与える.
    - [拡張（obs_augment=True のとき末尾に追加）]
        - dice_counts(6): 出目1..6の個数/5
        - upper_sum_norm(1): min(one_roles_sum,63)/63
        - upper_slots_left_norm(1): (未使用の上段枠数)/6
        - lower_slots_left_norm(1): (未使用の下段枠数)/6
    """
    ...
    def __init__(
        ...
        extra = 9 if self.obs_augment else 0
    ...
    # マスク（MaskablePPO は env.env_method("action_masks") を呼ぶ）
